  ...
  function setupConnection(socket) {
    socket.on('ishout-authentication', function(authData) {
      ...
      auth.authenticate(authData.authString, function(userId){
        ...
        // custom events for CAanalytics
        // consider moving them to a separate file
        socket.on('user.tool', function(group, msg) {
          handler.broadcastGroup(group, 'user.tool', msg);
        });
        socket.on('view.request', function(user, data) {
          handler.emit(user, 'view.request', data);
        });
        socket.on('view.stream', function(group, data) {
          for (var i = 0; i < data.watching.length; i++)
            handler.emit(data.watching[i], 'view.stream', data);
        });
        socket.on('view.stop', function(user, data) {
          handler.emit(user, 'view.stop', data);
        });
